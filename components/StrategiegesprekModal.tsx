...
const STEPS: Step[] = ["uitdaging", "bedrijfsgrootte", "budget", "timeline", "contact", "boek"];
const MAIN_STEP_COUNT = STEPS.filter((s) => s !== "budget-laag").length;
...
export default function StrategiegesprekModal({ open, onClose }: Props) {
  ...
  const currentIndex = isLowBudget ? 3 : stepIndex(step);
  ...
  function pick(key: keyof Answers, value: string) {
    ...
    const flow: Step[] = ["uitdaging", "bedrijfsgrootte", "budget", "timeline", "contact", "boek"];
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg bg-[#f2f3f5] shadow-2xl">
        ...
        <div className="px-6 py-8">
          {step === "uitdaging" && (
            <StepCard
              eyebrow={`Stap 1 van ${MAIN_STEP_COUNT - 1}`}
          ...
          {step === "bedrijfsgrootte" && (
            <StepCard
              eyebrow={`Stap 2 van ${MAIN_STEP_COUNT - 1}`}
          ...
          {step === "budget" && (
            <StepCard
              eyebrow={`Stap 3 van ${MAIN_STEP_COUNT - 1}`}
            ...
            >
              <div className="flex flex-col gap-3 mt-2">
                ...
                >
                  <span className="flex items-center gap-3">
                    <span>💼</span>
                    Volg ons op LinkedIn voor gratis AI-tips
                  </span>
                ...
                >
                  <span className="flex items-center gap-3">
                    <span>✉️</span>
                    Stuur een e-mail - misschien kunnen we verwijzen
                  </span>
                  <span className="text-black/30">→</span>
                </a>
                <button
                  className="mt-2 text-xs text-[#575760] underline hover:text-[#1f1f1f] transition-colors text-left"
                  onClick={back}
                >
                  ← Budget aanpassen
                </button>
          ...
          {step === "timeline" && (
            <StepCard
              eyebrow={`Stap 4 van ${MAIN_STEP_COUNT - 1}`}
        ...
        {step !== "uitdaging" && step !== "budget-laag" && step !== "boek" && (
          <div className="border-t border-black/8 px-6 py-4">
            <button
              onClick={back}
              className="text-xs text-[#575760] hover:text-[#1f1f1f] transition-colors"
            >
              ← Vorige stap
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-1">
        ...
        {subtitle && (
          <p className="text-sm text-[#575760] leading-relaxed">{subtitle}</p>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <a
          href={`mailto:info@maisonblender.com?subject=Strategiegesprek aanvraag${answers.bedrijf ? ` - ${answers.bedrijf}` : ""}&body=Naam: ${answers.naam ?? ""}%0ANaam bedrijf: ${answers.bedrijf ?? ""}%0AUitdaging: ${answers.uitdaging ?? ""}%0ABedrijfsgrootte: ${answers.bedrijfsgrootte ?? ""}%0ABudget: ${answers.budget ?? ""}%0ATimeline: ${answers.timeline ?? ""}`}
      ...
      <p className="text-xs text-[#b2b2be]">
        Geen verplichtingen. Geen pitch. Gewoon eerlijk advies.
      </p>
    </div>
  );
}
